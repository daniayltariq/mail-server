...
use PBMail\Smtp\Server\Auth\CramMd5Method;
use PBMail\Smtp\Server\Auth\LoginMethod;
use PBMail\Smtp\Server\Auth\MethodInterface;
use PBMail\Smtp\Server\Auth\PlainMethod;
use PBMail\Smtp\Server\Event\ConnectionAuthAcceptedEvent;
use PBMail\Smtp\Server\Event\ConnectionAuthRefusedEvent;
use PBMail\Smtp\Server\Event\ConnectionChangeStateEvent;
use PBMail\Smtp\Server\Event\ConnectionFromReceivedEvent;
use PBMail\Smtp\Server\Event\ConnectionHeloReceivedEvent;
use PBMail\Smtp\Server\Event\ConnectionLineReceivedEvent;
use PBMail\Smtp\Server\Event\ConnectionRcptReceivedEvent;
...
//                $this->sendReply(504, 'Unrecognized authentication type.');
    ...
    protected function handleMailFromCommand($arguments)
    {
        ...
        if (preg_match('/:\s*\<(?<email>.*)\>( .*)?/', $arguments, $matches) == 1) {
            // Check if mail from is one of our imported domains
            // IMPORTANT: if both of email_from and email_to is our domain, then we will save the email directly to db.
            // Otherwise this code will not work for receiving email.
            $dbHelper = DbHelper::getInstance();
            $domain = $dbHelper->findDomainShort($matches['email']);
            // If domain is from our domain then this means our users try to send email.
            // Require authentication for this request
            if($domain){
                // if user is not authenticated and we have auth methods defined then force authentication
    ...
    protected function handleRcptToCommand($arguments)
    {
        ...
        if (preg_match('/:\s*(?<name>.*?)?\<(?<email>.*)\>( .*)?/', $arguments, $matches) == 1) {
            // If both of MAIL FROM and RECIPIENT are not from our domain then reject the request.
            $dbHelper = DbHelper::getInstance();
            $fromDomain = $dbHelper->findDomainShort($this->from);
            $toDomain = $dbHelper->findDomainShort($matches['email']);
